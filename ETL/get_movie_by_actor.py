...
api_df = pd.read_json('movie_ids.json')
id = api_df
# create a function to get the movie ids by actor ids from the json file : "id_movie_by_actor.json"
def get_the_movies_by_actor_id(api_key):
    """_summary_ : This function returns the list of all the movies ids by actors ids listed in the json file "id_movie_by_actor.json" from the TMDB API

    Returns:
        list_of_movies: List of all the movies id by actors id listed in the json file "id_movie_by_actor.json" from the TMDB API
    """
    movie_data_list = []
    for id in api_df['id']:
        ...
        try:
            data = response.json()
            movie_data_list.append(data['id'])
        except:
            print('N/A')
...
def extract_movie_data_from_imdb_id(api_key_tmdb, id_list):
...
movies_ids = get_the_movies_by_actor_id(api_key_tmdb)
df = extract_movie_data_from_imdb_id(api_key_tmdb, movies_ids)
...
with open('actors_movies.json', 'w', encoding='utf-8') as f:
